...
class QueryGenerator {
  ...
  getOrderBy() {
    return 'ORDER BY ' + this.query.orderBy.join(', ');
  }
  ...
  static createMaterializedView(queryInterface, materializedViewName, model, options) {
    options = options || {};
  ...
  static buildDocument(includeOrModel, attributes, options) {
    ...
    return model.describe().then(modelDescription => {
      ...
      if (isInclude) {
        let foreignKey = QueryGenerator.col(include.foreignKey, options.modelDescription[include.targetKey] ?  options.tableName : model.tableName);
        let targetKey = QueryGenerator.col(include.targetKey, options.modelDescription[include.targetKey] ? model.tableName : options.tableName);
